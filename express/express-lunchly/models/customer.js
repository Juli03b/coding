...
class Customer {
  ...
  static async searchName(name){
    ...
    const customers = results.rows.map(row => {
      const {id, firstName, lastName, phone, notes} = row;
  ...
  static async topCustomers(){
    const results = await db.query(
      ...
      phone, 
      c.notes
      FROM reservations JOIN customers c on customer_id = c.id
      GROUP BY customer_id, c.id, first_name, last_name, phone, c.notes
